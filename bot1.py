...
BOT_TOKEN = '8330742827:AAFZvk2O_7NFxpDQBPnoL4GpoKL7yTE5CtE'
...
def load_all_data():
    ...
    try:
        ...
        # 1. Узлы воронки
        cursor.execute("SELECT node_id, node_key, message_text, image_url, is_root FROM funnel_nodes WHERE is_active = 1 AND bot_id = 2")
        nodes.clear()
        ...
        # 3. Reply-кнопки
        cursor.execute("SELECT button_text, response_text, node_key, message_key FROM bot_buttons WHERE bot_id = 2 AND is_active = 1 ORDER BY menu_order")
        reply_buttons.clear()
