...
BOT_TOKEN = '8781858038:AAGDIzxjBDpgo7N7Zy3Xjeu0puokHxrxwi8'
...
def load_all_data():
    ...
    try:
        ...
        # 1. Узлы воронки
        cursor.execute("SELECT node_id, node_key, message_text, image_url, is_root FROM funnel_nodes WHERE is_active = 1 AND bot_id = 3")
        nodes.clear()
        ...
        # 3. Reply-кнопки
        cursor.execute("SELECT button_text, response_text, node_key, message_key FROM bot_buttons WHERE bot_id = 3 AND is_active = 1 ORDER BY menu_order")
        reply_buttons.clear()
